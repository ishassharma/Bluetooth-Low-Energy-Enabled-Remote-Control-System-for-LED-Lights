...
//           think "#if (expression)".
#if (LOWEST_ENERGY_MODE == 0) //cannot sleep if in run mode
...
SL_WEAK void app_init(void)
{
  ...
  if ((LOWEST_ENERGY_MODE > SL_POWER_MANAGER_EM0) && (LOWEST_ENERGY_MODE < SL_POWER_MANAGER_EM3))
    {
      sl_power_manager_add_em_requirement(LOWEST_ENERGY_MODE); //setting energy mode
    }
...
//  switch (evt)
//  {
//    case evt_LETIMER0_UF:
//                          ReadWriteTempFromSI7021();
//                          break;
//  }
//
//  state_machine(evt);
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  // SERVER
  handle_ble_event_server(evt); // put this code in ble.c/.h
  // sequence through states driven by events
  keypad_server_state_machine(evt); // put this code in scheduler.c/.h
  #else
  //CLIENT
  handle_ble_event_client(evt); // put this code in ble.c/.h
