...
void displayInit()
{
    ...
    //
    TempSensorOn(); // we need SENSOR_ENABLE=1 which is tied to DISP_ENABLE
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  uint16_t Client_Handle = getBleDataPtr()->handle_for_client;

	  //software update warning, shoudl work for now
	  timer_response = sl_bt_system_set_soft_timer(SOFT_TIMER_VALUE,
	                                               Client_Handle,
	                                               false);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("sl_bt_system_set_soft_timer() returned != 0 status=0x%04x", (unsigned int) timer_response);
     }
